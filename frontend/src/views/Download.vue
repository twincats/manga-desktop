<template>
  <a-spin :loading="pageLoading" tip="This may take a while...">
    <div
      id="download"
      class="m-1 min-h-[calc(95vh-2rem)]"
    ...
    >
        ...
        <a-tab-pane :key="1" title="Download">
          <div class="px-2 my-2">
            <a-input
              ...
              :input-attrs="{
                class: 'text-center',
                style: '--default-contextmenu: auto;',
              }"
              placeholder="URL"
              allow-clear
            >
          ...
          >
            <div class="mb-2" style="background-color: var(--color-bg-2)">
              <div class="text-lg p-2 text-center min-h-56px">
                <div v-if="chapterList">{{ chapterList.manga }}</div>
              </div>
              <div class="flex">
                <div class="w-165px h-full px-2">
                  <m-image
                    v-if="chapterList"
                    :height="coverHeight"
                    width="165px"
                ...
                <div class="p-2 self-end" v-if="chapterList">
                  <a-space>
                    <a-button
                      status="normal"
                      @click="
                        ;(progress_dl.modal_log = true),
                          (logModal.center = true),
                          (logModal.mask = true),
                          (logModal.footer = true)
                      "
                      >Show Log</a-button
                    >
                    <a-button
      ...
      <!-- log modal -->
      <a-modal
        v-model:visible="progress_dl.modal_log"
        title="Log Modal"
        width="80%"
        :top="logModal.top"
        :align-center="logModal.center"
        :footer="logModal.footer"
        :mask="logModal.mask"
        :modal-style="{ colorScheme: 'dark' }"
        @close="
          () => {
            logModal.center = false
            logModal.mask = false
            logModal.footer = false
          }
        "
      >
        <div ref="reflog" class="overflow-auto h-150px lg:h-350px">
          <ul>
            <li v-for="(item, index) in log_dl" :key="index" v-html="item"></li>
          </ul>
        </div>
      </a-modal>
    </div>
...
import { useDownloadState, useMangaState, useLogState } from '@/store'
...
const { log, log_clear, log_dl } = useLogState()
...
const coverHeight = mangaState.getLimit('165px', '185px')
const pageLoading = ref(false)
const logModal = reactive({
  top: mangaState.getLimit('60%', '50%'),
  center: false,
  mask: false,
  footer: false,
})

const reflog = ref<HTMLElement | null>(null)
const { y } = useScroll(reflog)
watch(log_dl, () => {
  y.value += 20
})
// FOR NEXT PROGRESS
  ...
  if (server && urldata.value != '') {
    ...
    dl.GetChap(o, server)
      .then(res => {
        ...
        log(
          `Fetch Chapter List URL : ${o.url} - Server : ${o.server_name}`,
          'Success'
        )
      })
      .catch(err => {
        ...
        log(
          `Failed Fetch Manga URL : ${o.url} - Server : ${o.server_name}`,
          'Danger'
        )
      })
      .finally(() => {
    ...
    dl.GetPage(param, server)
      .then(res => {
        ...
        if (res.status_dl == false || res.fail_chap.length > 0) {
          ...
          log(
            `Partial download Manga : ${res.manga} - FailChap : ${res.fail_chap} - Error : ${res.error}`,
            'Fail'
          )
        } else {
          progress_dl.modal_dl = false
          progress_dl.modal_log = false
          Message.success('Berhasil Download')
          log(`All download Complete`, 'Success')
...
EventsOn('dl_eventchap', (p: EventChap) => {
  log(new EventChap(p)) // log chap change
...
EventsOn('dl_eventpage', (p: EventPage) => {
  log(new EventPage(p)) //log page status
